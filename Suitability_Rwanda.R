...
## Températures :
temp_mean = read_csv2('data/input/daily_avg_temp.csv')
colnames(temp_mean) = sapply(strsplit(colnames(temp_mean), "_"), `[`, 1)
day_temp = read_csv2('data/input/daily_temperature.csv')
...
## Incidence Rwanda :
incidence = read.xlsx('data/incidence_data/Incidence_Malaria_Rwanda_OMS.xlsx',sheetIndex = 1)
...
## Précipitations :
day_rain = read_csv2('data/input/daily_rain.csv')
...
rm(wst_mean_rain)
write.csv2(wst_mean,'data/output/WeatherStation_LongTermMean.csv',row.names = FALSE)
...
## Par station :
wst_y_gdd = day_temp %>% group_by(weather_station,year) %>% summarise(gdd_sum = sum(GDD))
ggplot(wst_y_gdd) +
  aes(x=year,y=gdd_sum) +
...
ggplot(wst_alt_temp2) +
  ...
  geom_smooth(method='lm', formula= y~x, color ='red') +
  xlab('Altitude (m)') + ylab('Température moyenne (°C)') +
  theme_bw() + theme(plot.title = element_text(hjust = 0.5))
lm_loocv(wst_alt_temp2,wst_alt_temp2$Altitude,wst_alt_temp2$temp_mean) #RMSE = 0.67°C
summary(reg_alt_temp)

## Régression pour 4 périodes => 1983 - 1991 ; 1992 - 2000 ; 2001 - 2008 ; 2009 - 2016 :
#Regression 1
...
lm_loocv(wst_alt_1,wst_alt_1$Altitude,wst_alt_1$temp_mean) #RMSE = 0.69 °C
...
lm_loocv(wst_alt_2,wst_alt_2$Altitude,wst_alt_2$temp_mean) #RMSE = 0.70°C
...
lm_loocv(wst_alt_3,wst_alt_3$Altitude,wst_alt_3$temp_mean) #RMSE =  0.68°C
...
lm_loocv(wst_alt_4,wst_alt_4$Altitude,wst_alt_4$temp_mean) #RMSE = 0.62°C
...
###        Code adapté depuis Villena et al. (2022)         ###
###############################################################

## Données pour An. Gambiae et P. Falciparum :
load("data/villena_data/Angamb_Pfalc_samps.Rsave")  #Échantillons de Villena
...
} #Fonction de favorabilité

############ Courbe de favorabilité en fonction de la température ############ 
...
## Plot de favorabilité :
st_max = max(na.omit(ST.M)) #Favorabilité maximale possible
ST.M.rel = ST.M/st_max      #Favorabilité relative => 1 désignant la température la plus favorable possible (~24.7°C)
...
write.csv2(ST.gen,'data/output/Suitability_Classify.csv')
Suitability_temp = ST.gen

############ Courbe de favorabilité pour le Rwanda ############

# Remarque : Favorabilité moyenne journalière pour une année type
...
## Plot de favorabilité :
ST.M.rel = ST.M/st_max #Favorabilité relative 
ST.q95.rel = ST.q95/st_max
ST.q5.rel = ST.q5/st_max
ST.gen = as.data.frame(temp)
ST.gen$M = ST.M.rel
...
#Écart de favorabilité par station
...
# Remarque : La boucle produit un csv de la favorabilité pour chaque station
...
for (w in 2:length(temp_mean)){
  ...
  file_path = paste('data/output/station_output/St_',colnames(temp_mean[w]),'.csv',sep='')
  write.csv2(ST.gen,file = file_path)
}

############ Analyse par station de la suitability ############

### Boucle avec analyse par période pour chaque station ###
## Listes des outputs :
...
for (w in 1:(length(temp_mean)-1)){
  ...
  #### Data : 
  file_path = paste('data/output/station_output/St_',colnames(temp_mean[w+1]),'.csv',sep='')
...
## Exemple d'observation des résultats :
...
ggplot(data_disp, aes(x = DIS_change, y = AUC_change)) +
  ...
  labs(x = "Changement de NJS (%)", y = "Changement d'ASC (%)") +
  theme_bw() + xlim(-100, 500) + ylim(-100, 500)

## Favorabilité moyenne pour chaque période par station :
#Tests de Student
...
## Outputs : 
write.csv2(x = Season_output, file = 'data/output/ST_Seasonal_Change.csv')
...
write.csv2(x = resume_ST, file = 'data/output/ST_Change_1983_2016.csv')
